...
use beibei\service\elasticsearch\index\MartshowMizheIndexService;


class Martshow_mizhe_controller extends Task_base {
    ...
    public function __construct() {
        // 设置当前索引脚本日志文件名
        $this->log_file_name = MartshowMizheIndexService::$_ES_INDEX_LOG_NAME;
        parent::__construct();
    }
    ...
    public function execute() {
        ...
        // NOTE：调用顺序不能颠倒
        $MartshowMizheIndexService = MartshowMizheIndexService::get_instance();
        // 创建一个新索引
        $createRes = $MartshowMizheIndexService->createIndex();
        ...
        // 索引数据
        $_loop = $MartshowMizheIndexService->buildDataToEs($this->last_id, self::$PAGE_NUM);
        $this->last_id = $MartshowMizheIndexService->getLastId();
        ...
        // 创建别名，让新索引提供服务
        $setRes = $MartshowMizheIndexService->setAlias();
        if (!$setRes) {
        ...
        // 删除旧索引
        $delres = $MartshowMizheIndexService->deleteIndex();
        if (!$delres) {
