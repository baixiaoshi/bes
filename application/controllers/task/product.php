...
use beibei\service\elasticsearch\index\ProductIndexService;


class Product_controller extends Task_base {
    ...
    public function __construct() {
        // 设置当前索引脚本日志文件名
        $this->log_file_name = ProductIndexService::$_ES_INDEX_LOG_NAME;
        parent::__construct();
    }
    ...
    public function execute() {
        ...
        // NOTE：调用顺序不能颠倒
        $ProductIndexService = ProductIndexService::get_instance();
                // 创建一个新索引
        $createRes = $ProductIndexService->createIndex();
        ...
        // 索引数据
        $_loop = $ProductIndexService->buildDataToEs($this->last_id, self::$PAGE_NUM);
        $this->last_id = $ProductIndexService->getLastId();
        ...
        // 创建别名，让新索引提供服务
        $setRes = $ProductIndexService->setAlias();
        if (!$setRes) {
        ...
        // 删除旧索引
        $delres = $ProductIndexService->deleteIndex();
        if (!$delres) {
