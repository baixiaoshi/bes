...
class Checkcode {
    ...
    function show() {
        ...
        $this->CI->session->set_userdata('_checkcode_', $code);
        @ob_clean();
    ...
    function showScript() {
        //显示验证码
        echo "var img_src = '/imgauthcode/show/?';n";
    ...
    function check($auth_code = null) {
        if ($this->CI->session->userdata('_checkcode_') && $auth_code) {
            if (strtolower($this->CI->session->userdata('_checkcode_')) === strtolower($auth_code)) {
                $this->CI->session->unset_userdata('_checkcode_');
                return TRUE;
            }
        }
        return false;
