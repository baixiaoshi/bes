...
class QQClient {
    ...
    function get_user_info($appid, $appkey, $access_token, $access_token_secret, $openid) {
        ...
        $o = new QQOauth();
        $info = $o->do_get($url, $appid, $appkey, $access_token, $access_token_secret, $openid);
        $arr = array();
...
class QQOauth {
    ...
    function get_request_token($appid, $appkey) {
        ...
        $sigstr = "GET" . "&" . $this->QQConnect_urlencode("http://openapi.qzone.qq.com/oauth/qzoneoauth_request_token") . "&";
    ...
    function redirect_to_login($appid, $appkey, $callback) {
        ...
        get_instance()->session->set_userdata("qq_oauth_token_secret", $result["oauth_token_secret"]);
        ////构造请求URL
        $redirect .= "oauth_token=" . $result["oauth_token"] . "&oauth_callback=" . $this->QQConnect_urlencode($callback);
        return $redirect;
    }
    ...
    function get_access_token($appid, $appkey, $request_token, $vericode) {
        //请求具有Qzone访问权限的access_token的接口地址, 不要更改!!
        $url = "http://openapi.qzone.qq.com/oauth/qzoneoauth_access_token?";
        
        //生成oauth_signature签名值。签名值生成方法详见（http://wiki.opensns.qq.com/wiki/【QQ登录】签名参数oauth_signature的说明）
        //（1） 构造生成签名值的源串（HTTP请求方式 & urlencode(uri) & urlencode(a=x&b=y&...)）
        $sigstr = "GET" . "&" . $this->QQConnect_urlencode("http://openapi.qzone.qq.com/oauth/qzoneoauth_access_token") . "&";
        ...
        //（2）构造密钥
        $key = $appkey . "&" . get_instance()->session->userdata("qq_oauth_token_secret");
        get_instance()->session->unset_userdata('qq_oauth_token_secret');
    ...
    function QQConnect_urlencode($str) {
        ...
        for ($x = 0; $len--; $x++) {
            if (($str[$x] < '0' && $str[$x] != '-' && $str[$x] != '.') || ($str[$x] < 'A' && $str[$x] > '9') || ($str[$x] > 'Z' && $str[$x] < 'a' && $str[$x] != '_') || ($str[$x] > 'z' && $str[$x] != '~')) {
                $urlencode .= '%';
    ...
    function QQConnect_urldecode($str) {
        ...
        for ($x = 0; $x < $len; $x++) {
            if ($str[$x] == '%' && ($len - $x) > 2 && (strpos($QQhexchars, $str[$x + 1]) !== false) && (strpos($QQhexchars, $str[$x + 2]) !== false)) {
                $tmp = $str[$x + 1] . $str[$x + 2];
